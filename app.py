...
# --- 2. 套件檢查 (保留 PDF 支援，但主要靠內建功能讀 TXT) ---
pdf_ready = False
try:
    import pdfplumber
    pdf_ready = True
...
def load_kb():
    ...
    for f in txt_files:
        if "requirements.txt" in f: continue # 跳過設定檔
        try:
            with open(f, "r", encoding="utf-8") as file:
                content = file.read()
                full_text += f"\n=== 文件: {f} ===\n{content}\n"
                count += 1
                debug_log.append(f"✅ 成功載入 TXT: {f}")
        except Exception as e:
            debug_log.append(f"❌ TXT 讀取失敗 {f}: {e} (請確認是 UTF-8 編碼)")
            ...
            try:
                with pdfplumber.open(f) as pdf:
                    ...
                    for page in pdf.pages:
                        ...
                        if extracted: pdf_content += extracted + "\n"
                    full_text += f"\n=== PDF文件: {f} ===\n{pdf_content}\n"
                    count += 1
                    debug_log.append(f"✅ 成功載入 PDF: {f}")
            except Exception as e:
                debug_log.append(f"❌ PDF 讀取失敗 {f}: {e}")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 📚 知識庫 (支援 TXT/PDF)")
    if st.session_state.kb_count > 0:
        st.success(f"✅ 已掛載 {st.session_state.kb_count} 份文件")
    else:
        st.info("ℹ️ 資料夾內無 .txt 或 .pdf 檔案")
    
    with st.expander("🔍 檔案狀態檢查"):
        for m in st.session_state.kb_debug:
            st.write(m)
        if st.button("🔄 重新掃描檔案"):
    ...
    if apikey:
        ...
        try:
            ...
            ms.sort(key=lambda x: "1.5-flash" not in x) # Flash 排第一
...
if st.session_state.current_strategy:
    ...
    if q := st.chat_input("輸入問題..."):
        ...
        try:
            r = generate_with_retry(model, full_p)
